...
func ValidateServingInfo(info api.ServingInfo) errs.ValidationErrorList {
	...
	if len(info.ServerCert.CertFile) > 0 {
		if _, err := os.Stat(info.ServerCert.CertFile); os.IsNotExist(err) {
			allErrs = append(allErrs, errs.NewFieldInvalid("certFile", info.ServerCert.CertFile, "does not exist"))
		}

		if len(info.ServerCert.KeyFile) == 0 {
			allErrs = append(allErrs, errs.NewFieldRequired("keyFile", info.ServerCert.KeyFile))
		} else if _, err := os.Stat(info.ServerCert.KeyFile); os.IsNotExist(err) {
			allErrs = append(allErrs, errs.NewFieldInvalid("keyFile", info.ServerCert.KeyFile, "does not exist"))
		}

		if len(info.ClientCA) > 0 {
			if _, err := os.Stat(info.ClientCA); os.IsNotExist(err) {
				allErrs = append(allErrs, errs.NewFieldInvalid("clientCA", info.ClientCA, "does not exist"))
			}
		}
	} else {
		if len(info.ServerCert.KeyFile) > 0 {
...
func ValidateNodeConfig(config *api.NodeConfig) errs.ValidationErrorList {
	...
	} else if _, err := os.Stat(config.MasterKubeConfig); os.IsNotExist(err) {
		allErrs = append(allErrs, errs.NewFieldInvalid("masterKubeConfig", config.MasterKubeConfig, "does not exist"))
	}
