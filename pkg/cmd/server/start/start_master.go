...
func (o MasterOptions) RunMaster() error {
	...
	if o.WriteConfigOnly {
		...
		if err := configapi.ResolvePaths(configapi.GetMasterFileReferences(masterConfig), cwd); err != nil {
			return err
		}
		...
		if err := configapi.RelativizePaths(configapi.GetMasterFileReferences(masterConfig), base); err != nil {
			return err
		}
...
func ReadMasterConfig(filename string) (*configapi.MasterConfig, error) {
	...
	if err := configapi.ResolvePaths(configapi.GetMasterFileReferences(config), base); err != nil {
		return nil, err
	}
