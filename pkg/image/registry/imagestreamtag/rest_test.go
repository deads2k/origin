		...
		func() {
			...
			if testCase.image != nil {
				client.Put(
					...
					runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.GroupVersion), testCase.image),
				)
			}
			if testCase.repo != nil {
				client.Put(
					...
					runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.GroupVersion), testCase.repo),
				)
			}
...
func TestGetImageStreamTagDIR(t *testing.T) {
	...
	client.Put(
		...
		runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.GroupVersion), image),
	)
	client.Put(
		context.TODO(),
		etcdtest.AddPrefix("/imagestreams/default/test"),
		runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.GroupVersion), repo),
	)
	obj, err := storage.Get(apirequest.NewDefaultContext(), "test:latest", &metav1.GetOptions{})
	if err != nil {
	...
	for name, tc := range tests {
		func() {
			...
			client.Put(
				context.TODO(),
				etcdtest.AddPrefix("/imagestreams/default/test"),
				runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.GroupVersion),
